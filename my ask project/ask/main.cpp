...
// fill user question is afiltraion way to filter from user and to user Questions
	void FillUserQuestions(User &user) {
		...
		for (auto &pair : questionid_questionidsThead_to_map) {	// pair<int, vector<Question>>
			for (auto &question_id : pair.second) {		//  vector<Question>
				...
				// if the this Question from me bushback to from me vector
				if (question.from_user_id == user.user_id)
					user.questions_id_from_me.push_back(question.question_id);

                // if the this Question to me bushback to the right palce in the map if havem't parent or not
	...
	//chaeck if  i can answring that question from any user
	...
	// Used to ask a question on a specific thread for whatever user even if me
...
struct UsersManager {
	map<string, User> userame_userobject_map;//i'm here 3
	User current_user;
	int last_id;
...
struct AskMeSystem {
	...
	void LoadDatabase(bool fill_user_questions = false) {
		users_manager.LoadDatabase();//i'm here 2
	...
	void run() {
		LoadDatabase();// load user manger and questions and fetch to and from user question
		users_manager.AccessSystem();// do login and sign up
		questions_manager.FillUserQuestions(users_manager.current_user);
